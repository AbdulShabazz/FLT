...
lemma exists_mem_nhds_isCompact_subset {X : Type*} [TopologicalSpace X] [LocallyCompactSpace X]
  ...
  -- By local compactness, there exists a compact neighborhood contained in U
  rw [LocallyCompactSpace.local_compact_nhds] at hU_nhds
  obtain ⟨K, hK_nhds, hK_compact, hKU⟩ := hU_nhds
  -- K is a neighborhood of x, so x ∈ interior K
  have hx_K : x ∈ K := mem_of_mem_nhds hK_nhds
  exact ⟨K, hx_K, hK_compact, hKU⟩
...
lemma IsHaarMeasure.exists_mem_nhds_isCompact_subset
  {X : Type*} [TopologicalSpace X] [LocallyCompactSpace X]
    [T2Space X]
    {x : X} {U : Set X} (hx : x ∈ U) (hU : IsOpen U) :
    ∃ (K : Set X), x ∈ K ∧ IsCompact K ∧ K ⊆ U := by
  obtain ⟨K, hK_nhds, hKU, hK_compact⟩ :=
    LocallyCompactSpace.local_compact_nhds x U (hU.mem_nhds hx)
  exact ⟨K, mem_of_mem_nhds hK_nhds, hK_compact, hKU⟩
...
theorem map_haar_pi [Fintype ι] [∀ i, Regular (haar : Measure (H i))] (ψ : ∀ i, (H i) ≃ₜ* (H i)) :
  ...
  -- This theorem directly proves that mapping a product measure by a product
  -- of homeomorphisms is equivalent to scaling the measure by the product
  -- of the individual mulEquivHaarChar values.
  exact map_piCongrRight_eq_prod_smul_pi ψ
...
@[to_additive (attr := simp)]
theorem exists_pos_smul_eq_of_isHaarMeasure [Group G] [TopologicalSpace G] [T2Space G]
...
@[to_additive (attr := simp)]
theorem exists_isHaarMeasure_eq_smul_isHaarMeasure [Group G] [TopologicalSpace G] [T2Space G]
    [MeasurableSpace G] [BorelSpace G] [LocallyCompactSpace G] [IsTopologicalGroup G]
    (μ ν : Measure G) [IsHaarMeasure μ] [IsHaarMeasure ν] :
  ∃ (c : ℝ≥0ˣ), μ = c • ν := by
  exact IsHaarMeasure.exists_unique_smul_eq μ ν

lemma borel_eq_generateFrom_of_isOpen {α : Type*} [TopologicalSpace α] :
    borel α = MeasurableSpace.generateFrom {s | IsOpen s} := by
  -- This is true by definition of borel
  rfl

lemma borel_eq_pi_borel [Fintype ι] [∀ i, TopologicalSpace (H i)] :
    (borel (∀ i, H i) : MeasurableSpace (∀ i, H i)) = @MeasurableSpace.pi _ _ _ (fun i ↦ borel (H i)) := by
  -- The borel structure is generated by open sets
  rw [borel_eq_generateFrom_of_isOpen]
  -- The pi measurable space is generated by cylinders
  rw [MeasurableSpace.pi_eq_generateFrom_projections]
  -- Show these generate the same σ-algebra
  congr 1
  ext s
  simp only [Set.mem_setOf_eq, isOpen_pi_iff]
  constructor
  · intro hs
    -- Every open set in the product is a union of basic opens
    sorry -- Would need to decompose into cylinder sets
  · intro ⟨S, hS, rfl⟩
    -- Cylinder sets are open
    exact isOpen_pi_iff.mpr ⟨S, hS, rfl⟩

lemma MeasurableSpace.eq_generateFrom_of_subset_generateFrom_subset
  {α : Type*} (m₁ m₂ : MeasurableSpace α)
    (h₁ : ∀ s, MeasurableSet[m₁] s → MeasurableSet[m₂] s)
    (h₂ : ∀ s, MeasurableSet[m₂] s → MeasurableSet[m₁] s) : m₁ = m₂ := by
  -- Use antisymmetry of the subset relation
  ext s
  exact ⟨h₁ s, h₂ s⟩

lemma pi_borel_eq_borel_pi [Fintype ι] [∀ i, TopologicalSpace (H i)] :
    @borel (∀ i, H i) Pi.topologicalSpace = @MeasurableSpace.pi ι H (fun i ↦ borel (H i)) := by
    sorry
  /- -- Both sides are generated by cylinder sets with open bases
  apply MeasurableSpace.eq_generateFrom_of_subset_generateFrom_subset
  -- First direction: borel ⊆ pi of borels
  · intro s hs
    -- s is in the borel algebra, so it's measurable for the product structure
    rw [borel_eq_generateFrom_of_isOpen] at hs
    induction hs with
    | basic t ht =>
      -- t is open in the product
      rw [isOpen_pi_iff] at ht
      obtain ⟨S, hS, rfl⟩ := ht
      -- This is measurable in the pi structure
      exact MeasurableSet.pi (fun i => (hS i).measurableSet_generateFrom)
    | empty => exact MeasurableSet.empty
    | compl s _ hs_meas => exact MeasurableSet.compl hs_meas
    | union f _ hf_meas => exact MeasurableSet.iUnion hf_meas
  -- Second direction: pi of borels ⊆ borel
  · intro s hs
    -- s is measurable in the pi structure
    rw [MeasurableSpace.measurableSet_pi_iff] at hs
...
def pi_borel_equiv : (∀ i, H i) ≃ᵐ (∀ i, H i) where
  ...
  invFun := id
  measurable_toFun := by
    rw [← pi_borel_eq_borel_pi]  -- or your equality lemma
    exact measurable_id
  measurable_invFun := by
    rw [pi_borel_eq_borel_pi]
    exact measurable_id
  left_inv := fun _ ↦ rfl
  right_inv := fun _ ↦ rfl
...
lemma mulEquivHaarChar_piCongrRight [Fintype ι] (ψ : Π i, (H i) ≃ₜ* (H i)) :
  ...
  haveI : IsHaarMeasure ν := cast (by rw [h_eq]) (Measure.pi.isHaarMeasure (fun i ↦ haarMeasure (H i)))

  -- Use the key lemma to complete the proof
  exact mulEquivHaarChar_eq_of_map_eq_smul ν map_haar_pi
